...
// Node.js çalıştırma ortamını zorunlu kılıyoruz
...
export async function GET() {
  try {
    const fetchedEmails = await new Promise<any[]>((resolve) => {
    ...
    return NextResponse.json({ success: true, emails: fetchedEmails });
  } catch (error: any) {
    return NextResponse.json({ success: false, error: error.message, emails: [] });
  }
}